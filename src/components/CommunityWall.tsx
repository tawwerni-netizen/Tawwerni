...
import { COMMUNITY_300, CommunityMember } from "@/content/community-300";
  ...
  return (
    <section className="py-12 px-4 sm:px-6">
      ...
      <div className="mx-auto max-w-3xl text-center mb-10">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-teal-500/30 bg-teal-500/10 px-3.5 py-1 text-xs font-bold text-teal-300 mb-3">
          <span>👥</span>
      ...
      <div className="mx-auto max-w-5xl mb-8 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        ...
        <div className="relative sm:w-64">
          <input
            ...
            className="w-full rounded-full border border-neutral-800 bg-neutral-900 px-4 py-2 text-xs text-white placeholder-neutral-500 focus:border-teal-500 focus:outline-hidden"
          />
        </div>
      </div>
      ...
      <div className="mx-auto max-w-5xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {visibleMembers.map((member) => (
          <div
            key={member.id}
            className="rounded-3xl border border-neutral-800/80 bg-neutral-900/60 p-5 backdrop-blur-md flex flex-col justify-between hover:border-teal-500/30 transition-all hover:-translate-y-1 shadow-md text-white"
          >
            <div>
              {/* Member Top Bar */}
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-teal-400 text-xs" title={t.verifiedLearner}>✓</span>
                  </div>
              ...
              <div className="flex items-center gap-1 mb-2.5 text-amber-400 text-xs">
                <span>★★★★★</span>
                <span className="text-neutral-500 font-mono text-[10px]">({member.rating})</span>
              </div>

              {/* Quote */}
              <p className="text-xs text-neutral-300 leading-relaxed mb-4 italic">
                "{lang === "ar" ? member.quoteAr : member.quoteEn}"
              </p>
            </div>

            {/* Track Tag Footer */}
            <div className="pt-3 border-t border-neutral-800/60 flex items-center justify-between text-[11px] text-neutral-400">
              <span className="inline-flex items-center gap-1 text-teal-300 font-medium truncate max-w-[200px]">
                <span>📚</span>
                <span className="truncate">{lang === "ar" ? member.trackTitleAr : member.trackTitleEn}</span>
              </span>
              <span className="text-[10px] text-neutral-500 font-mono">Verified</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {visibleCount < filteredMembers.length && (
        <div className="mt-8 text-center">
          <button
            ...
            className="px-6 py-2.5 rounded-full border border-teal-500/40 bg-teal-500/10 hover:bg-teal-500/20 text-teal-300 text-xs font-bold transition-all hover:scale-105 active:scale-95"
          >
            {lang === "ar"
